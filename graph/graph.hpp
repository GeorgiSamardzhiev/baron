...
namespace baron {
	...
	struct Edge {
		...
		Edge(const Vertex& _vertex, Weight _weight) :vertex(_vertex), weight(_weight) {};

	};

	template<typename Vertex, typename Weight>
	bool operator<(const Edge<Vertex, Weight>& e1, const Edge<Vertex, Weight>& e2) {
				...
				if (marked.count(edge.vertex) == 0) {
					...
					try {
						for (Edge<Vertex, Weight> e : this->at(edge.vertex)) {
							...
							if (marked.count(e.vertex) == 0 && e.weight + dist[edge.vertex] < dist[e.vertex]) {
								priorityQueue.push(Edge<Vertex, Weight>(e.vertex, dist[e.vertex] = e.weight + dist[edge.vertex]));
							}
						}
					} catch (...) {}
				}
				...
				if (marked.count(edge.vertex) == 0) {
					...
					try {
						for (Edge<Vertex, Weight> e : this->at(edge.vertex)) {
							...
							if (marked.count(e.vertex) == 0 && e.weight < dist[e.vertex]) {
								priorityQueue.push(Edge<Vertex, Weight>(e.vertex, dist[e.vertex] = e.weight));
							}
						}
					} catch (...) {}
				}
